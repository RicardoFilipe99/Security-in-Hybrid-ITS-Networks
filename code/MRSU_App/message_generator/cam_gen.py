...
class MessageGenerator:
    """
    CAM message generator
    """

    def __init__(self, mqtt_dispatcher, rsu_dispatcher, sec_prot, evaluator):
        self.mqtt_dispatcher = mqtt_dispatcher
        self.rsu_dispatcher = rsu_dispatcher
        ...
        while True:
            ...
            if self.evaluator.evaluation_mode == EvaluationEnum.TotalComputationTime:
                self.evaluator.execution_performance(
                    execution_lambda=lambda: self.send_message(),
                    identifier=f'GeneratedToMQTTandITS_G5_'
                               f'{SecurityProtocolEnum[self.sec_prot.protocol_designation.name]}'
                               f'[TimesInMs]',
                )
            # ---------------- ---------------- ---------------- ---------------- ----------------
    ...
    def send_message(self):
        """
        Send message via G5 and to broker
        """
        # Generate message
        ...
        if self.evaluator.evaluation_mode == EvaluationEnum.SecurityComputationTime:
            self.evaluator.execution_performance(
                execution_lambda=lambda: self.sec_prot.apply_security_protocol_encapsulation(
                    its_message=msg),
                identifier=f'MessageEncapsulation_'
                           f'{SecurityProtocolEnum[self.sec_prot.protocol_designation.name]}'
                           f'[TimesInMs]',
            )
        # ---------------- ---------------- ---------------- ---------------- ----------------

        secure_msg = self.sec_prot.apply_security_protocol_encapsulation(
            its_message=msg
        )

        logging.debug(f"Sending generated message via MQTT")
        self.mqtt_dispatcher.dispatch_to_mqtt(message=secure_msg, message_type='CAM')

        # --------- Send via ITS-G5 --------- #
        security_fields_bytes = self.sec_prot.get_security_fields_bytes(
            encapsulated_its_message=secure_msg)

        # Include security bytes in original message
        secure_msg = include_security_fields_in_cam(its_message=msg,
                                                    security_fields_bytes=security_fields_bytes)

        secure_msg = binascii.hexlify(secure_msg).decode()

        logging.debug(f"Sending generated message via G5")
        self.rsu_dispatcher.dispatch_to_rsu(message=secure_msg, message_type='CAM')

        # Evaluation zone: ---------------- ---------------- ---------------- ----------------
        if self.evaluator.evaluation_mode == EvaluationEnum.TransmissionToG5NetworkDelay:
...
def generate_cam(static):
    """
    CAM generator (it may return a static message or build new ones)
    ...
    try:
        if static:
            return '0202000003e803e8005997d7ed8cbb585e401401401430d5400000000000003e713a800bffe5fff800'
